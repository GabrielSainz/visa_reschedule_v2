...
def start_process():
    ...
    # auto_action("Arrow down bounce", "xpath",
    #             '//a[@class="down-arrow bounce"]', "click", "", STEP_TIME)
    # auto_action("Login start", "xpath",
    #             '//*[@id="header"]/nav/div[2]/div[1]/ul/li[3]/a', "click", "", STEP_TIME)

    # Wait(driver, 60).until(EC.presence_of_element_located((By.NAME, "commit")))

    auto_action("Click bounce", "xpath",
                '//a[@class="down-arrow bounce"]', "click", "", STEP_TIME)
    auto_action("Email", "id", "user_email", "send",
                USERNAME, random.randint(1, 3))
    auto_action("Password", "id", "user_password",
                "send", PASSWORD, random.randint(1, 3))
    auto_action("Privacy", "class", "icheckbox",
...
def reschedule(asc_date):
    asc_time = get_time(asc_date)
    ...
    if(r.text.find('Successfully Scheduled') != -1):
        title = "SUCCESS"
        msg = f"Rescheduled Successfully! {asc_date} {time}"
    else:
        title = "FAIL"
        msg = f"Reschedule Failed!!! {asc_date} {time}"
    return [title, msg]


def get_date(asc_flag=False):
    # Requesting to get the whole available dates
    driver.get(ASC_DATE_URL)

    if not is_logged_in():
        start_process()
        return get_date(asc_flag)
    else:
